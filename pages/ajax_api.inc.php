...
function rex_markitup_imm_imgtypes()
{
    ...
    $db = rex_sql::factory();
    $query = rex_register_extension_point('REX_MARKITUP_IMAGE_TYPES_QUERY', 'SELECT * FROM ' . $REX['TABLE_PREFIX'] . '679_types ' . $REX['ADDON']['gs_markitup']['settings']['imm_sql_where'] . ' ORDER BY `name` ASC');                                                       //FB::log($query,' $query');
